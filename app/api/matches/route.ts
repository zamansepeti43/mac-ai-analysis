...
async function sportmonksRequest<T>(path: string, withXg: boolean): Promise<T[]> {
  const token = process.env.SPORTMONKS_API_TOKEN;
  if (!token) return [];
  const include = withXg ? "participants;league;xGFixture" : "participants;league";
  const response = await fetch(`${SPORTMONKS_URL}${path}&include=${include}`, { headers: { Authorization: token }, next: { revalidate: 300 } });
...
async function sportmonks<T>(path: string): Promise<T[]> {
  ...
  // xGFixture gibi opsiyonel bir include plan tarafından reddedilirse gerçek fixture'ları yine göster.
  return sportmonksRequest<T>(path, false);
}
...
async function getSportmonksMatches(from: string, to: string, limit: number) {
  const path = from === to ? `/fixtures/date/${from}?per_page=50` : `/fixtures/between/${from}/${to}?per_page=100`;
  const fixtures = await sportmonks<SportmonksFixture>(path);
  const now = Date.now();
  return fixtures.filter((fixture) => new Date(fixture.starting_at).getTime() >= now - 30 * 60 * 1000).sort((a, b) => new Date(a.starting_at).getTime() - new Date(b.starting_at).getTime()).slice(0, limit).map(mapSportmonksFixture);
}
...
export async function GET(request: Request) {
  ...
  if (process.env.SPORTMONKS_API_TOKEN) {
    ...
    return NextResponse.json({ source: "sportmonks", range, from, to, matches, note: "Gerçek fixture verisi kullanılır; opsiyonel xG erişilemiyorsa fixture listesi yine gösterilir." });
  }
